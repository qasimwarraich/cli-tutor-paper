\chapter{User Study}
\section{Section}
%
\subsubsection{Subsubsection}

\paragraph{Paragraph.} Always with a point.

\begin{figure}[H]
	\scalebox{0.75}{\input{plots/programming_exp/plot.pgf}}
	\caption{The distribution of programming experience amongst study participants}
	\label{fig:programmingexp}
\end{figure}

\begin{figure}[H]
	\scalebox{1}{\input{plots/uni_exp/plot.pgf}}
	\caption{University level Computer Science experienced amongst study participants}
	\label{fig:uniexp}
\end{figure}

\begin{figure}
	\centering
	\begin{minipage}{0.45\textwidth}
		\centering
		\scalebox{0.55}{\input{plots/uni_exp/plot.pgf}}
		\caption{first figure}
	\end{minipage}\hfill
	\begin{minipage}{0.45\textwidth}
		\centering
		\scalebox{0.55}{\input{plots/uni_exp/plot.pgf}}
		\caption{second figure}
	\end{minipage}
\end{figure}

\begin{figure}
	\centering
	\begin{minipage}{0.45\textwidth}
		\centering
		\scalebox{0.55}{\input{plots/programming_exp/plot.pgf}}
		\caption{first figure}
	\end{minipage}\hfill
	\begin{minipage}{0.45\textwidth}
		\centering
		\scalebox{0.55}{\input{plots/uni_exp/plot.pgf}}
		\caption{second figure}
	\end{minipage}
\end{figure}


\begin{figure}[H]
	\scalebox{1}{\input{plots/comfort/plot.pgf}}
	\scalebox{0.8}{\input{plots/comfort/plot.pgf}}
	\caption{spam}
	\label{fig:uniexp}
\end{figure}
