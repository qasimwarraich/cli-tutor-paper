...
custom parser for our lesson files. The parser first parses a selected lesson file into an \textit{AST}
(Abstract Syntax Tree). Once this tree is created, it is traversed using a
\textit{Depth First Search} in order to navigate to all the different syntax
nodes in the tree. In our lesson specification we make divisions using heading
weights in our \textit{Markdown} files. 
...
binary. This was achieved using a compiler directive built into the \textit{Go}
...
Interfaces and is inspired by the \textit{Elm}
architecture\footnote{\url{https://guide.elm-lang.org/architecture/}}. The
\textit{Elm} architecture is a pattern that consists of three part structure
where messages are sent between parts to alter state, update the user interface
...
\item \textit{View:} The \textit{View} is the last part of the \textit{Elm}
    ...
    consists of a blocking \textit{for} loop.
